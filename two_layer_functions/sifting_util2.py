...
def update_positions(free_layer, minimized_layer, pos, layer_height=2):
    """
    Update the positions of nodes in the free layer based on the minimized layer order.

    Args:
        free_layer (list): List of node labels in the free layer.
        minimized_layer (list): List of node labels in the minimized layer order.
        pos (dict): Dictionary containing the current positions of the nodes.
        layer_height (int, optional): Vertical spacing between layers. Defaults to 2.

    Returns:
        dict: Updated positions of the nodes, this is thew new pos.
    """
    # Create a deepcopy of the positions to avoid modifying the original
    updated_pos = copy.deepcopy(pos)
    
    # Update positions for the nodes in the minimized layer
    x_offset = -(len(minimized_layer) - 1) / 2  # Center nodes horizontally
    for i, node in enumerate(minimized_layer):
        updated_pos[node] = (x_offset + i, updated_pos[node][1])  # Update x-coordinate, keep y-coordinate

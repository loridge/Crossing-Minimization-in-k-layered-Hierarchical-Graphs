...
def count_crossings(B, pos):
    ...
    crossings = 0
    edges = list(B.edges())
    # print("bipartite graph generator", edges)
    ...
    for n1 in df["n1"].unique():
        for n2 in df["n2"].unique():
            ...
            if not subset.empty:
                ...
                # if subset["crossings_branch_and_cut"]: plt.plot(x, subset["crossings_branch_and_cut"], label="Branch and cut Heuristic", marker="x")

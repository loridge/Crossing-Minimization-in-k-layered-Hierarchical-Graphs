...
def sifting(free_layer: Union[Set[str], List[str]], fixed_layer: Union[Set[str], List[str]], edges: list, verbose=0, LA_gen=0) -> list:
    """
    Perform the sifting algorithm to reorder the free layer nodes to minimize edge crossings.
    ...
    # print("isfting",free_layer, fixed_layer, edges)
    
    # print("major sifting:", free_layer, "fixed",fixed_layer, edges)
    # print("BEFORE testing crossing inside sift2:", cross_count(fixed_layer, free_layer, edges))
    reordered_layer = sifting_2(free_layer, fixed_layer, edges, verbose)
    # print("After testing crossing inside sift2:", cross_count(fixed_layer, reordered_layer, edges))
    # print("reordered sifting", reordered_layer)
    # if generated:
    #     reordered_layer = [int(node[1:]) for node in reordered_layer]
    
    
    # LELEY, cinomment out ko muna yung mga code na to para gumana sa 2 Layer Experiment. use generated=1 pa rin lagi   
    ## commented out to follow new LA implementation + new bip generator.

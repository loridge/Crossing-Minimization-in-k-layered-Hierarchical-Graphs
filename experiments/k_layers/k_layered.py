...
def save_k_layered_graph(G, layers, title, num_crossings, save_dir = "k_layered_graphs", filename=None):
    """
    Visualizes a k-layered directed graph with distinct vertical layers,
    similar to the bipartite visualization style.
    ...
    plt.suptitle(f"Crossings: {num_crossings}")
    ...
    # Save the figure
    if filename is None:
        filename = f"{title.replace(' ', '_')}_k-layered.png"  # Default filename based on the title

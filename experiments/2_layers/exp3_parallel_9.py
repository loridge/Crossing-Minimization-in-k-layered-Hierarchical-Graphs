...
def plot_results_percentage(df, message=""):
    """
    ...
    for n1 in df["n1"].unique():
        for n2 in df["n2"].unique():
            ...
            if not subset.empty:
                ...
                # y_min = min(y_barycenter.min(), y_median.min(), y_sifting.min(), )
                # y_max = max(y_barycenter.max(), y_median.max(), y_sifting.max(), )
                
                # Round min/max to nearest 0.5
                y_min = np.floor(y_min * 2) / 2
                y_max = np.ceil(y_max * 2) / 2

                # print("Debug error", y_min, y_max)
...
if __name__ == '__main__':
    ...
    print(f"EXP3:Executing {n1}-{n2} nodes ")
    # Experiment loop
    with ProcessPoolExecutor() as executor:
        ...
        #     for n2 in n2_values:
        for p in p_values:
            futures.append(executor.submit(run_experiment, n1, n2, p, num_samples))
